...
def compute_accuracy(predicts,labels):
    accuracy = 0
    accuracy += tf.reduce_sum(tf.reduce_mean(tf.cast(tf.equal(tf.argmax(predicts,1),tf.argmax(labels,1)),dtype=tf.float32)))
    # print(type(predicts))
    # print(labels.shape)
    return accuracy
...
log_dir = 'F:/Pycharm/tensorflow/Visualization/log1'
...
with tf.Session() as sess:
    ...
    for epoch in range(100):
        ...
        summary = sess.run(merged,feed_dict={X_input:X_input1,Y_input:Y_input1})
